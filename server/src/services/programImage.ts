...
function buildHtml(raid: any, club: any): string {
  const accent = club.accentColor ?? '#c0392b'
  const dateStr = format(new Date(raid.date), "dd 'DE' MMMM 'DE' yyyy", { locale: pt }).toUpperCase()
  const points: any[] = raid.routePoints ?? []
  ...
  points.forEach((p: any) => {
    ...
    if (p.dayOffset) {
      ...
      const dayDateStr = format(dayDate, "dd 'de' MMMM 'de' yyyy", { locale: pt })
      scheduleRows.push(`
...
async function launchAndRender(html: string): Promise<{ browser: any; page: any }> {
...
export async function generateProgramImage(raid: any, club: any): Promise<Buffer> {
...
export async function generateProgramPDF(raid: any, club: any): Promise<Buffer> {
  const resolvedLogoUrl = resolveImageForPuppeteer(club.logoUrl)
  club = { ...club, logoUrl: resolvedLogoUrl }
  const html = buildHtml(raid, club)
  const { browser, page } = await launchAndRender(html)
