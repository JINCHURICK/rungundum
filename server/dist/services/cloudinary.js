...
exports.calibrateClockOffset = calibrateClockOffset;
...
async function calibrateClockOffset() {
    const sources = [
        ...
        async () => {
            ...
            if (!dateHeader)
                throw new Error('sem Date header');
...
async function _doCloudinaryUpload(buffer, folder, resourceType, extraOptions = {}) {
    function doUpload(timestamp) {
        return new Promise((resolve, reject) => {
            const stream = cloudinary_1.v2.uploader.upload_stream({ folder, resource_type: resourceType, timestamp, ...extraOptions }, (error, result) => {
                if (error)
                    return reject(error);
    ...
    const withTimeout = (p) => Promise.race([
        p,
        new Promise((_, reject) => setTimeout(() => reject(new Error('Cloudinary upload timeout (30s)')), 30_000)),
    ]);
    try {
        ...
        if (typeof err?.message === 'string' && err.message.includes('Stale request')) {
            ...
            if (correctedTs)
                return await withTimeout(doUpload(correctedTs));
        }
        throw err;
    }
}
