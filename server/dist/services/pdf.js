...
function buildMemberProfileHTML(member, club, accentColor, resolvedLogoUrl) {
    ...
    const quotas = member.quotas ?? [];
    const currentYear = new Date().getFullYear();
    const currentQuota = quotas.find((q) => q.year === currentYear);
    const fmtDate = (d) => d ? (0, date_fns_1.format)(new Date(d), 'dd/MM/yyyy') : '—';
    const fmtMonth = (d) => d ? (0, date_fns_1.format)(new Date(d), "MMMM 'de' yyyy", { locale: locale_1.ptBR }) : '—';
    const statusMap = { ACTIVE: 'Activo', INACTIVE: 'Inactivo', SUSPENDED: 'Suspenso', GUEST: 'Convidado' };
    const statusBg = { ACTIVE: '#dcfce7', INACTIVE: '#f1f5f9', SUSPENDED: '#fef2f2', GUEST: '#eff6ff' };
    const statusFg = { ACTIVE: '#16a34a', INACTIVE: '#64748b', SUSPENDED: '#dc2626', GUEST: '#2563eb' };
    const st = member.status ?? 'ACTIVE';
    const vehicleTypeLabel = (t) => ({ TRAIL: 'Trail', ROAD: 'Estrada', CUSTOM: 'Custom', SCOOTER: 'Scooter', SUPPORT: 'Apoio', OTHER: 'Outro' }[t] ?? t);
    return `
