...
exports.DEFAULT_ALERT_CONFIG = void 0;
exports.getAlertLevel = getAlertLevel;
exports.runQuotaAlerts = runQuotaAlerts;
...
function getAlertLevel(monthsOverdue, cfg) {
    if (monthsOverdue >= cfg.suspensionRiskMonths)
        return 4;
    if (monthsOverdue >= cfg.disciplinaryMonths)
        return 3;
    if (monthsOverdue >= cfg.secondAlertMonths)
        return 2;
    if (monthsOverdue >= cfg.firstAlertMonths)
        return 1;
    return 0;
}
// ── utilitário: meses em atraso no ano corrente ───────────────────────────────
function calcMonthsOverdue(monthsPaid, quotaYear) {
    const now = new Date();
...
// ── função principal ──────────────────────────────────────────────────────────
async function runQuotaAlerts(clubId) {
    ...
    for (const club of clubs) {
        ...
        const cfg = { ...exports.DEFAULT_ALERT_CONFIG, ...(settings.quotaAlertConfig ?? {}) };
        if (!cfg.enabled)
            continue;
        // membros activos com quota no ano corrente
        const quotas = await prisma_1.prisma.memberQuota.findMany({
            where: {
        ...
        for (const quota of quotas) {
            ...
            // enviar SMS
            if (phone) {
                await (0, sms_1.sendQuotaReminderSms)({
                    phone,
                    memberName,
                    clubName: club.name,
                    year: quota.year,
            ...
            // enviar email
            if (email) {
                await (0, email_1.sendQuotaAlertEmail)({
                    to: email,
                    memberName,
                    clubName: club.name,
                    year: quota.year,
            ...
            if (level >= 3) {
                ...
                const existing = await prisma_1.prisma.disciplinaryProcess.findFirst({
                ...
                if (!existing) {
                    ...
                    if (admin) {
                        await prisma_1.prisma.disciplinaryProcess.create({
                            data: {
                                clubId: club.id,
                                memberId: member.id,
                                title,
            ...
            await prisma_1.prisma.memberQuota.update({
                where: { id: quota.id },
...
// ── helpers de config ─────────────────────────────────────────────────────────
async function getAlertConfig(clubId) {
    const club = await prisma_1.prisma.club.findUnique({ where: { id: clubId }, select: { defaultSettings: true } });
    const s = (club?.defaultSettings ?? {});
    return { ...exports.DEFAULT_ALERT_CONFIG, ...(s.quotaAlertConfig ?? {}) };
}
async function saveAlertConfig(clubId, cfg) {
    const club = await prisma_1.prisma.club.findUnique({ where: { id: clubId }, select: { defaultSettings: true } });
    const s = (club?.defaultSettings ?? {});
